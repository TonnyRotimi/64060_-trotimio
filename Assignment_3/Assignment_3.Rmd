...
```{r}
Target_client <- data.frame(Online = 1, CreditCard = 1)
```

Predict the default status of test dataset
```{r}
Predicted_Test_label <-predict(nb_model, Target_client, type="raw")
Predicted_Test_label
```

Probability (Loan = 1 | CC = 1, Online = 1)?
```{r}
prob_3<- Predicted_Test_label[1, "1"]
```

Compare this to the number you obtained in (E).
Result fromcalculations in E
```{r}
prob_1
prob_3
```
...
The result from running the model gives the probability of a client who took a personal loan, has a credit card and is using online services is 0.116, while the result of the computation is 0.0640.
